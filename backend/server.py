...
def plan_path():
    ...
    # Use the first boundary point as the reference for coordinate conversion
    ref_lat = data['boundary']['points'][0]['latitude']
    ref_lon = data['boundary']['points'][0]['longitude']
    
    # Convert all LatLng coordinates to a local meter-based grid
    ...
    # A-Star planner expects boundary as min/max corners
    ...
    for obs in data['obstacles']:
        if obs['type'] == 'rectangle':
            ...
            # Radius is given in meters, no conversion needed.
